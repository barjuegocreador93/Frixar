        ...
        function Using(packagesName) {
          function str_(namestr)
          {
            ...
            if(psing.cursor.$type=='srv')
            {
                ...
                var attrperims = {app:{type:'module',attrs:['Name']},factoryService:{type:'extension',attrs:['Name','extension']},controller:{type:'cntrl',attrs:['Name']}};
                srv.extension.$methods.FindAllByType=function(type,result){
                  srv.Root().ForAll(function(v,r){
                    if(attrperims[type])
                    if(v.HasType(attrperims[type].type))
                      {
                        var ob= {};
                        for (var p of attrperims[type].attrs)
                        {
                          ob[p] = v[p];
                        }
                        r.push(ob);
